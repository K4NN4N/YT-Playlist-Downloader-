...
while true; do
    ...
    yt-dlp -f bestaudio --extract-audio --audio-format "$audio_format" --audio-quality "$audio_quality" $thumbnail_option $reverse_option --add-metadata --output "$download_directory/%(playlist_index)02d %(title)s.%(ext)s" "$playlist_url"
    ...
    case $post_download_choice in
        1)
            ...
            ;;
        2)
            yt-dlp -f bestaudio --extract-audio --audio-format "$audio_format" --audio-quality "$audio_quality" $thumbnail_option $reverse_option --add-metadata --output "$download_directory/%(playlist_index)02d %(title)s.%(ext)s" --download-archive "$archive_file" --force-overwrites --retries 3 --ignore-errors --continue "$playlist_url"
            ;;
        3)
